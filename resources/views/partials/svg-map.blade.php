<!--banner/hududiy yangiliklar-->
<div id="locals" class="container-fluid">
	<div class="container py-3">
		<div class="row">
            <div v-if="isLoading" class="col-6 my-auto" style="z-index: 1000; top: 50%; left: 20%;">
                <div class="spinner-grow" style="width: 5rem; height: 5rem; background-color: antiquewhite;" role="status">
                    <span class="sr-only">Loading...</span>
                </div>
            </div>
			<div v-else class="col-6">
				<div class="row border-bottom border-danger text-white">
					<h4 class="text-uppercase"><b>{{  __('Региональные новости') }}</b></h4>
					<a class="ml-auto text-reset" href="javascript:void(0);">{{ __('Все') }}</a>
				</div>
				<div v-if="posts" class="row text-white pt-3">
                    <a v-for="(post, i) in posts" v-if="i < 6" class="col-6 text-white text-decoration-none" :href=`/pages/single_post/${post.id}/{{ app()->getLocale() }}`>
                        <div class="media">
                            <div class="media-body">
                                <h6 class="mt-0" v-text="post.title"></h6>
                                <p class="text-warning small" v-text="post.author.organization.oblast.nameru"></p>
                            </div>
                        </div>
                    </a>
				</div>
			...
			<div class="col-6">
				<div class="row">
                    <select
                        ...
                        class="selectpicker form-control form-control-sm ml-auto"
                        data-width="auto"
                        @change="changeColorBySelectOption">
						<option selected value="">{{  __('Все регионы') }}</option>
                        <option
                            v-for="region in regions"
                            :value="region.regionid"
                            v-text="region.nameru">
                        </option>
					</select>
				</div>
				<br>
...
@section('locals-vue-scripts')
    <script>
        let localsApp = new Vue({
            el:'#locals',
            ...
            methods: {
                getUzRegions() {
                    this.regions = store.state.regions;
                },

                changeColorByClickOnArea(el) {
                    let allArea = document.querySelectorAll('.map-highlight');

                    allArea.forEach(element => element.attributes.fill.value = '#fff');
                    el.target.attributes.fill.value = 'grey';
                    this.regionSelect = parseInt(el.target.id);
                },

                changeColorBySelectOption(el) {
                    let allArea = document.querySelectorAll('.map-highlight');
                    let path = document.getElementById(this.regionSelect);

                    allArea.forEach(element => element.attributes.fill.value = '#fff');
                    path.attributes.fill.value = 'grey'
                }
            },

            watch: {
                regionSelect: function(params) {
                    this.isLoading = true;

                    axios('{{ route("filtered_posts") }}', {params: {"regionId": params}})
                        .then(response => {
